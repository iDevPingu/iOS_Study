...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        let firstView = TodoListViewController()
        let secondView = SettingViewController()
        tabBarController.viewControllers = [firstView,secondView]
        let firstItem = UITabBarItem(title: "Tasks", image: .checkmark, tag: 0)
        let secondItem = UITabBarItem(title: "Setting", image: .actions, tag: 1)
