...
> ### Q : 상태 변화에 따라 다른 동작을 처리하기 위한 앱델리게이트 메서드들을 설명하시오.
 - A : [App LifeCycle](https://icksw.tistory.com/189), [App LifeCycle 변화 발생시 호출되는 AppDelegate, SceneDelegate Methods](https://icksw.tistory.com/245)
 - `applicationDidBecomeActive(UIApplication)`, `sceneDidBecomeActive(UIScene)`
   - 앱이 Active 상태가 되었을 때 호출되는 메서드입니다.
 - `applicationWillEnterForeground(UIApplication)`, `sceneDidEnterForeground(UIScene)`
 ...
 - `applicationWillResignActive(UIApplication)`, `sceneWillResignActive(UIScene)`
   - 앱이 Active 상태를 포기했을 때 호출되는 메서드 입니다. (Active -> In-Active)
 - `applicationWillTerminate(UIApplication)`
   - 앱이 종료되기 직접에 호출되는 메서드 입니다.
...
> ### Q : 앱이 In-Active 상태가 되는 시나리오를 설명하시오.
 ...
 - 이러한 과정에서 Scene을 사용할 때와 Scene을 사용하지 않을 때의 호출되는 메서드가 다릅니다.
...
> ### Q : NotificationCenter 동작 방식과 활용 방안에 대해 설명하시오.
 - [NotificationCenter 사용법](https://icksw.tistory.com/209)
 - A : NotificationCenter는 특정 객체에 옵저버를 추가하여 어떤 상황이 발생했을 때 해당 옵저버에 알람을 보내면 특정 작업을 수행하게 됩니다. NotificationCenter는 앱을 만들 때 기본적으로 default라는 NotificationCenter가 만들어지며 새로운 NotificationCenter를 만들어 사용할 수도 있습니다. 활용방안으로는 시스템적으로 발생하는 이벤트들에 대하여 옵저버를 추가해 알람을 받도록 만들 수 있습니다. 예를 들어 키보드가 올라올 때 키보드에 의해 가려지는 뷰를 처리한다던가, 앱이 백그라운드 상태가 되었을 때 뭔가 수행중이던 작업을 멈춘다거나 하는 등의 활용방법이 있습니다.
...
> ### Q : Safearea에 대해서 설명하시오.
 - A : 아이폰X가 나오게 되면서 상단과 하단영역이 곡선모양이 되었고 상단에는 노치가 존재하게 되며 디스플레이의 변화가 발생했습니다. 따라서 이러한 디스플레이가 없었던 시절에 만들어진 앱들이 아이폰X에서는 잘 나오지 않게되는 문제가 발생했습니다. 예를 들어 기존처럼 superview를 기준으로 앱을 만들면 노치에 의해 보이지 않게 되거나 화면이 잘려서 보이는 등의 문제가 발생하므로 잘 보일 수 있도록 safearea를 추가하여 이를 방지할 수 있게 되었습니다.
...
> ### Q : Reference Count 방식에 대해 설명하시오.
 - [ARC와 강한 참조 사이클(Strong Reference Cycle](https://icksw.tistory.com/204)
 - A : Reference Count란 클래스와 같은 참조 타입에서 객체를 참조하는 것들의 갯수를 나타냅니다. 강한 참조로 참조를 하게되면 Reference Count가 1씩 증가하며 약한 참조나 비소유(unowned) 참조로 참조하게 되면 Reference Count가 증가하지 않습니다. 참조 타입의 객체는 메모리에서 해제가 되기 위해선 Reference Count가 0이 되어야하며 만약 사용하지 않더라도 Reference Count가 1이상이라 메모리에서 해제가 되지 않는다면 이로 인해 메모리 누수가 발생할 수 있습니다.
...
> ### Q : Reference 과 assign 의 차이점을 설명하시오.
> ### Q : 순환 참조에 대하여 설명하시오.
> ### Q : 강한 순환 참조 (Strong Reference Cycle) 는 어떤 경우에 발생하는지 설명하시오.
 - [ARC와 강한 참조 사이클(Strong Reference Cycle](https://icksw.tistory.com/204)
 - A : 두 클래스의 프로퍼티가 서로의 클래스 타입일 때 발생할 수 있습니다. 예를 들어 A,B 클래스가 있고 A 클래스의 프로퍼티 중 B 클래스 타입이 존재하고 B 클래스의 프로터티 중 A 클래스 타입이 존재하는 상태에서 A,B 인스턴스를 만들고 각각의 프로퍼티로 서로를 참조하게 되면 강한 참조 주기가 발생합니다. 두 인스턴스를 nil로 할당해도 강한참조가 남아있기 때문에 참조 카운트가 0이 되지 않아서 메모리에 계속 남아있게 됩니다.
